...
/**
 * This panel implements a simple arcade game in which the user tries to blow up a "submarine" (a
 * black oval) by dropping "depth charges" (a red disk) from a "boat" (a blue roundrect). The user
 * moves the boat with the left- and right-arrow keys and drops the depth charge with the down-arrow
 * key. The sub moves left and right erratically along the bottom of the panel. This class contains
 * a main() routine to allow it to be run as a program.
 */
public class SubKillerOriginal extends JPanel {
    ...
    /**
     * The constructor sets the background color of the panel, creates the timer, and adds a
     * KeyListener, FocusListener, and MouseListener to the panel. These listeners, as well as the
     * ActionListener for the timer are defined by anonymous inner classes. The timer will run only
    ...
    /**
     * The paintComponent() method draws the current state of the game. It draws a gray or cyan
     * border around the panel to indicate whether or not the panel has the input focus. It draws
     * the boat, sub, and bomb by calling their respective draw() methods.
     */
    ...
    /**
     * This nested class defines the boat. Note that its constructor cannot be called until the
     * width of the panel is known!
     */
    private class Boat {
    ...
    /**
     * This nested class defines the sub. Note that its constructor cannot be called until the width
     * of the panel is known!
     */
