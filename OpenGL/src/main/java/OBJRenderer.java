...
public class OBJRenderer implements GLEventListener, MouseMotionListener, KeyListener {
  ...
  public void init(final GLAutoDrawable glad) {
    ...
    projection.perspective(
        (float) Math.toRadians(45), (float) width / (float) height, 0.1f, 100.0f);
  ...
  public void display(final GLAutoDrawable glad) {
    ...
    gl.glUniformMatrix4fv(
        gl.glGetUniformLocation(cubeShaderProgramID, "view"),
        1,
        false,
        viewMatrix.get(matrixBuffer));
    gl.glUniformMatrix4fv(
        gl.glGetUniformLocation(cubeShaderProgramID, "projection"),
        1,
        false,
        projection.get(matrixBuffer));
    gl.glBindVertexArray(skyboxVAO[0]);
