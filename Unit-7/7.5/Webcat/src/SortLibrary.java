...
public class SortLibrary {
    ...
    public static void main(String[] args) {
        ...
        int[] arrayToSort = mostlySorted; // arrayToSort will point to the array you choose
        int[] copyOfArrayToSort = Arrays.copyOf(arrayToSort, arrayToSort.length);

        // ***Enter which sort you want to test
        mergeSort(arrayToSort); // Call your sort method -- Remember array is modified in the
        // method, not returned!
    ...
    public static void mergeSort(int[] nums) {
        ...
        for(int blockSize = 1; blockSize < nums.length; blockSize *= 2) {
            for(int start = 0; start < nums.length; start +=2* blockSize) {
                int mid = Math.min(start + blockSize, nums.length);
                int high = Math.min(start + 2*blockSize, nums.length);
                merge(nums, temp, start,mid,high);
            }
        }
    }
