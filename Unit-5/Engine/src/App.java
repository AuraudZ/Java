...
public class App {
        public static void main(String[] args) {
                ...
                JSlider fovSlider = new JSlider(SwingConstants.VERTICAL, 0, 90, 45);
                pane.add(fovSlider, BorderLayout.WEST);
                        ...
                        public void paintComponent(Graphics g) {
                                ...
                                for (Triangle t : tris) {
                                        ...
                                        Vertex ab = new Vertex(v2.x - v1.x, v2.y - v1.y,
                                                        v2.z - v1.z);
                                        Vertex ac = new Vertex(v3.x - v1.x, v3.y - v1.y,
                                                        v3.z - v1.z);
                                        Vertex norm = new Vertex(ab.y * ac.z - ab.z * ac.y,
                                                        ab.z * ac.x - ab.x * ac.z,
                                                        ab.x * ac.y - ab.y * ac.x);
                                        double normalLength = Math.sqrt(norm.x * norm.x
                                        ...
                                        int minX = (int) Math.max(0, Math.ceil(
                                                        Math.min(v1.x, Math.min(v2.x, v3.x))));
                                        int maxX = (int) Math.min(img.getWidth() - 1, Math.floor(
                                                        Math.max(v1.x, Math.max(v2.x, v3.x))));
                                        int minY = (int) Math.max(0, Math.ceil(
                                                        Math.min(v1.y, Math.min(v2.y, v3.y))));
                                        int maxY = (int) Math.min(img.getHeight() - 1, Math.floor(
